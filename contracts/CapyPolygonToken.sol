...
contract CapyPolygonToken is ERC20, ERC20Burnable, Pausable, Ownable, ReentrancyGuard, NonblockingLzApp {
    // Constants
    ...
    function removeBridgeAddress(address _bridge) external onlyOwner {
        bridgeAddresses[_bridge] = false;
    }

    function bridgeOut(string calldata destinationChain, address recipient, uint256 amount) external whenNotPaused nonReentrant {
        ...
        // Emit event for bridge oracle to pick up
        emit BridgeTransfer(msg.sender, destinationChain, recipient, amount);
    }

    function bridgeIn(address to, uint256 amount, string calldata sourceChain) external whenNotPaused nonReentrant {
        require(bridgeAddresses[msg.sender], "Caller is not a bridge");
        ...
        emit TokensBridged(to, amount, sourceChain);
    }
    ...
    function calculateRewards(address user) public view returns (uint256) {
        ...
        uint256 rewardPerDay = (stakedBalance[user] * rewardRate) / 1000;
        return (rewardPerDay * timeElapsed) / STAKING_PERIOD;
    }
    ...
    function claimTeamTokens() external {
        ...
        require(block.timestamp >= teamVestingStart + TEAM_CLIFF_PERIOD, "Cliff period not over");

        uint256 totalVestingTime = block.timestamp - teamVestingStart;
        if (totalVestingTime > TEAM_VESTING_DURATION) {
            totalVestingTime = TEAM_VESTING_DURATION;
        }

        uint256 totalVestingAmount = TEAM_ALLOCATION * 10**decimals();
        uint256 vestedAmount = (totalVestingAmount * totalVestingTime) / TEAM_VESTING_DURATION;
        uint256 claimableAmount = vestedAmount - vestedAmount[teamWallet];

        if (claimableAmount > 0) {
            vestedAmount[teamWallet] = vestedAmount;
    ...
    function claimDevelopmentTokens() external {
        ...
        uint256 totalVestingAmount = DEVELOPMENT_ALLOCATION * 10**decimals();
        uint256 vestedAmount = (totalVestingAmount * totalVestingTime) / DEVELOPMENT_VESTING_DURATION;
        uint256 claimableAmount = vestedAmount - vestedAmount[developmentWallet];

        if (claimableAmount > 0) {
            vestedAmount[developmentWallet] = vestedAmount;
            _mint(developmentWallet, claimableAmount);
        }
    }
    ...
    function claimMarketingTokens() external {
        ...
        uint256 totalPeriods = (block.timestamp - marketingVestingStart) / MARKETING_VESTING_PERIOD;
        uint256 totalVestingAmount = MARKETING_ALLOCATION * 10**decimals();
        uint256 amountPerPeriod = totalVestingAmount / 4; // Release over 4 quarters

        uint256 vestedAmount = totalPeriods >= 4 ? totalVestingAmount : amountPerPeriod * totalPeriods;
        uint256 claimableAmount = vestedAmount - vestedAmount[marketingWallet];

        if (claimableAmount > 0) {
            vestedAmount[marketingWallet] = vestedAmount;
            _mint(marketingWallet, claimableAmount);
        }
    }

    // Admin functions
    function setTransferTaxRate(uint256 newRate) external onlyOwner {
        require(newRate <= 5, "Tax rate too high"); // Max 5%
    ...
    // Override transfer functions to implement tax and anti-whale
    function _transfer(
        address sender,
        address recipient,
        uint256 amount
        ...
        uint256 taxAmount = 0;
        if (transferTaxRate > 0 && !isExemptFromLimit[sender] && !isExemptFromLimit[recipient]) {
